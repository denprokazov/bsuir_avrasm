...
.org $0008 rjmp plusButtonInterrupt      ; (INT3) External Interrupt Request 3
.org $000A reti       ; (INT4) External Interrupt Request 4
.org $000C rjmp minusButtonInterrupt      ; (INT5) External Interrupt Request 5
...
plusButtonInterrupt:
	cpi counter, 0x99 ; compare counter to 99
	breq donePlusOperation ; if counter = 99 then act like a dead
	ldi operationSummand, 0x01 ; else operation plus
	bset  6; operation flag for counterEvent
donePlusOperation: reti

minusButtonInterrupt:
	cpi counter, 0x00 ; compare counter to 0
	breq doneMinusOperation ; if counter = 0 then act like a dead
	ldi operationSummand, 0xFF ; else set -1 in additive code
	bset 6
doneMinusOperation: reti

resetButtonInterrupt: ; if reset button is tapped
	bclr 6 	;	clear user flag
	clr counter ; clear counter
doneResetOperation: reti
...
init:
	...
	sts EICRA, temp
	ldi temp, 0b10001000 ; set up the falling edge mode on int5 and int 7
	sts EICRB, temp
